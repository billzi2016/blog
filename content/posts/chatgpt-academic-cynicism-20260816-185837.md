...
审稿人并不总是那个真正站在领域中心、对问题有完整理解、带着耐心和善意替学术共同体把关的人。很多时候，他只是一个其实并不了解这个细分方向，却因为自己发过论文、欠着 program committee 的人情、需要维持学术履历，或者为了凑 EB-1 / EB-2 材料里那些“peer review contribution”而不得不强行审稿的人。他报这个名，接这个活，表面上说是服务学术共同体，实际上的动机往往非常纯粹：我要留下记录，我要满足美国政府移民材料里的最低要求，我要证明我曾经作为同行评审过别人的工作。

很不幸的是，这个要求天然带着囚徒困境的味道。它有最低线，却没有真正的上限。你审了两篇，我可以审五篇；你审了五篇，我可以审十篇；你说你参与了国际会议评审，我就要参与更大的会议、更多的期刊、更长的 reviewer list。于是审稿不再只是判断论文质量，而变成一种可以被计数、被截图、被写进推荐信、被塞进申请材料里的功绩凭证。它越是被包装成“无私服务”，越像一套精致的功利积分系统。

另一种动机更简单，也更阴：零和博弈。尤其在顶会和强竞争期刊里，你投，我也投；你中，我的相对位置就低一点；你的论文活下来，我的论文就多一份被挤掉的可能。审稿人当然不会公开承认这一点，他会说自己只是严格、只是负责、只是维护标准、只是不能让不成熟的工作进入共同体。但在拥挤到近乎窒息的投稿系统里，把别人拒掉，本来就可能是自己胜出的间接条件。

所以审稿人这个角色本身就非常矛盾。他既是球员，又是裁判；既参与同一场名额竞争，又被要求假装自己站在竞争之外；既有非常具体、非常现实、非常功利的收益，又必须用一整套非功利的语言把自己包起来。他要在系统里抢资源、攒资历、保位置，却还要以“客观评审”“学术标准”“共同体责任”的名义发言。这个综合体一旦接上 AI，后面的事情就会变得更荒诞。

在当代学术建制的边缘，正在上演着一幕极为离奇而又极其默契的戏码。

#### A. 99.9% 人类无法直视的绝对黑盒

当今学术界有一个很尴尬的现状：很多论文并不是因为问题本身必须复杂，才写得复杂；而是因为它必须显得复杂，才被写成那样。一个简单想法如果直接说出来，像工程报告；套上一层范畴语言、再套一层随机过程、再接一段谱序列、再把优化问题写成泛函极值，立刻就像“前沿工作”。复杂性本来应该来自对象本身，来自真正绕不开的困难；但今天相当一部分复杂性，已经变成了一种学术装甲：越厚，越难被质疑；越晦涩，越像有深度；越没人能完整看懂，越方便在评审系统里活下来。

这不是说所有高阶数学、理论机器学习或统计物理都是装腔。真正的问题当然可能极其困难，真正的理论也当然需要精密语言。可现实里还有另一种东西：为了复杂而复杂，为了像论文而论文，为了让审稿人不敢轻易说“这其实没什么”而不断加符号、加定义、加定理、加附录。最后文本本身变成一堵墙，作者躲在墙后，审稿人站在墙前，谁也不完全知道里面到底有多少真实内容，多少只是防御工事。
...
这件事换成数字会更直观。第一个 AI 被要求生成一串“毫无规律”的二十位数字，于是它给出：

> 73920581647392018475

现在把这串数字交给第二个 AI，让它分析规律。它完全可以认真拆：

```text
73920 58164 73920 18475
```

前十位 `7392058164` 正好把 `0-9` 十个数字各用了一次，没有重复，也没有遗漏；如果把前十位切成两组，就是 `A = 73920` 和 `B = 58164`。接着第十一到第十五位又完整重复了一遍 `A`，于是前十五位变成了非常漂亮的 `A-B-A`。最后五位 `18475` 看起来也不是完全乱的：它从 `B` 里拿走了 `1、8、4、5`，只剩一个 `6` 没出现，又从 `A` 里借了一个 `7` 插进来，像是一次从 `B` 向 `A` 回摆的混合段。
...
然后问“下一位是什么”，第二个 AI 真的可以填 `6`。理由也能写得很顺：因为最后一段已经回收了 `B` 里的 `1、8、4、5`，唯独缺少 `6`；补上 `6` 以后，尾部就完成了对 `B` 的缺失项补全，同时也把前面 `A-B-A` 的结构重新拉回到 `B`。但是实际上，这种解读没有任何意义；在题目一开始就声明“毫无规律”的前提下，它不只是牵强，而是必然错误。它只是足够像一份评审意见：有分段，有命名，有局部重复，有缺失项，有一个可以被包装成“自然延续”的结论。

论文审稿也是同一套动作。你给它一篇论文，再给它一个 strong reject 的姿态，它就会从里面找出 strong reject 的规律；你给它一个 weak accept 的姿态，它也能从同一篇论文里找出 weak accept 的规律。它不是在判断论文，而是在把既定结论补成一段可以提交的文字。随机数字都能被补出下一位，论文当然也能被补出任何评价。

甚至可以把这个游戏玩得更狠一点：让 AI 去强拒四篇后来被认为极其重要的论文。它完全写得出来，而且会写得很像。

AlphaGo Zero 可以被它强拒成这样：文章只在围棋这个封闭、规则清晰、可无限自博弈的环境里展示结果，无法说明方法能迁移到真实世界的开放任务；相比已有 AlphaGo，主要改动是去掉人类棋谱、加大自博弈和搜索规模，概念创新不足；实验依赖巨量算力，没有给出普通研究者可复现的成本边界；所谓“从零开始”也只是从围棋规则开始，不能被包装成真正通用的智能机制。结论：interesting engineering, but limited scientific novelty.

AlphaFold 也可以被它强拒成这样：蛋白质结构预测长期依赖同源序列、模板信息、进化约束和领域先验，本文只是把这些东西用更大的神经网络重新组织了一遍；CASP 结果虽然漂亮，但 benchmark 场景和真实湿实验之间仍有距离；模型复杂到很难判断哪个组件真正贡献了性能；对无序区域、复合物、动态构象和细胞环境里的结构变化仍然处理不足。结论：impressive system integration, but the fundamental biological question remains unresolved.

Transformer 更好拒。AI 可以说，论文把 recurrence 和 convolution 都拿掉，只留下 attention，但没有充分证明这种结构在更长序列、更复杂推理和更大规模任务上仍然稳定；实验主要集中在机器翻译，任务范围太窄；self-attention 的二次复杂度会限制长度扩展；multi-head attention、position encoding、residual connection、layer normalization 拼在一起，到底哪一个带来关键收益并不清楚。结论：a neat simplification, but insufficiently validated as a general architecture.

CLIP 也逃不掉。它可以被写成：方法本质上是用四亿图文对做大规模对比学习，创新点被数据规模和算力规模掩盖；网页文本噪声很大，图像和文本之间未必有稳定语义对应；zero-shot 结果依赖 prompt 设计，评估协议容易被提示词工程影响；模型学到的是互联网共现关系，不等于真正理解视觉世界；偏见、版权、数据污染和不可复现的数据集都没有被充分解决。结论：promising empirical result, but too dependent on scale and uncontrolled web supervision.

这四段如果单独拿出来看，都不像纯粹胡说。每一句都有一点抓手，每一句都能在论文里找到某个局部证据，每一句都能让作者在 rebuttal 里花半页解释。可荒谬也正在这里：同一套话术足够把 AlphaGo Zero、AlphaFold、Transformer、CLIP 都写成 weak reject 或 borderline reject。AI 不需要理解这些工作后来为什么重要，它只要知道“现在我要拒”，就能把拒绝写成一种审慎、专业、负责、好像很懂的样子。
