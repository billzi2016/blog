...
在所有人初始财富完全平等、交易规则绝对无偏的封闭系统中，人们直觉上常以为财富会维持均等。然而，统计物理给出的结论却冷酷而颠覆：**只要设定财富非负（不允许负债），系统在长期演化后会自发走向极度不平等的指数财富分布。**

这种不平等并非来自能力差异、剥削或信息不对称，而是系统在守恒约束下追求**最大熵（MaxEnt）**的物理必然。

```mermaid
graph TD
    A["初始状态: 全员等额 (极低系统熵)"] --> B["完全随机对等交易 (热力学涨落)"]
    B --> C["非对称边界作用: 财富非负 (m ≥ 0)"]
    C --> D["左侧硬墙阻挡 (零点吸附/亏损截断)"]
    C --> E["右侧无界延伸 (概率空间无限累积)"]
    D --> F["稳态归宿: 玻尔兹曼-吉布斯指数分布 (最大熵状态)"]
    E --> F
    F --> G["理论基尼系数 G = 0.5 (纯粹自由市场的底线)"]
```
...
graph LR
    subgraph 自由随机模型
        M1["标准无偏交换"] --> R1["玻尔兹曼指数分布 (G = 0.5)<br>无中产阶级，底层高度挤压"]
    end
    subgraph 储蓄保护机制
        M2["固定储蓄率 λ > 0"] --> R2["Gamma 概率分布<br>中产阶级自发突现 (Middle Class)"]
    end
    subgraph 马太效应模型
        M3["Yard-Sale 按相对资产交易"] --> R3["财富凝聚 (Wealth Condensation)<br>极值黑洞 (G 趋近 1.0)"]
    end
```
